...
function getBackendExecutableName() {
  return process.platform === "win32" ? "disciples2-unit-editor-backend.exe" : "disciples2-unit-editor-backend";
}

function getPackagedBackendCommand() {
  const executableName = getBackendExecutableName();
  return path.join(process.resourcesPath, "backend", "disciples2-unit-editor-backend", executableName);
}
...
function findFreePort(startPort = 8000) {
  return new Promise((resolve, reject) => {
    ...
    server.listen(startPort, HOST, () => {
      ...
      server.close(() => reject(new Error("Failed to resolve a free backend port")));
    });
  });
}
...
async function startBackend() {
  ...
  const command = app.isPackaged ? getPackagedBackendCommand() : getDevBackendCommand();
